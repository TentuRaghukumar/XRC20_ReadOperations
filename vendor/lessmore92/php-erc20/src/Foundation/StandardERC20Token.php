...
// }
    ...
    public function approve(string $sender_address, string $reciever_address, string $amount,string $gasLimit = 'default', string $gasPrice = 'default')
    {
        $amount   = Number::scaleUp($amount, $this->decimals());
        $data     = $this->buildApproveData($reciever_address, $amount);
        $nonce    = Number::toHex($this->getEth()
                                       ->getTransactionCount($sender_address, 'pending'));
        // $gasLimit = $this->getGasLimit('approve');
        // $gasPrice = $this->getSafeGasPrice();
        if (strtolower($gasLimit) === 'default')
        {
